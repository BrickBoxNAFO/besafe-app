...
export { getResend }
export const FROM = process.env.RESEND_FROM_EMAIL || 'support@homesafeeducation.com'
  ...
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <div style="background:#F0F4F8;border-radius:12px;padding:20px 24px;margin:24px 0;">
        ...
        <p style="color:#374151;font-size:14px;line-height:1.7;margin:0;"><strong>3.</strong> Your <strong>${packageName}</strong> course unlocks instantly, so you can start whenever you're ready</p>
      </div>
...
export async function sendCertificateToPurchaser({ to, purchaserName, completedByName, courseName, pdfBuffer }) {
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <p style="color:#374151;font-size:16px;line-height:1.7;">Their Certificate of Completion is attached to this email. The safety knowledge they've gained is something they'll carry with them for life, and that's thanks to you.</p>
      ${infoBox(`<strong>${completedByName}'s certificate is attached as a PDF.</strong> You can download or print it to share with them in person.`)}
...
// ─── 5b. Invite Accepted Notification (to family owner) ──────────────────
export async function sendInviteAccepted({ to, ownerName, memberName, packageName }) {
  return getResend().emails.send({
    from: FROM, to,
    subject: `${memberName} accepted your HomeSafeEducation invitation`,
    html: wrap(`
      <h1 style="color:#0B1F3A;font-size:26px;margin:0 0 16px;">✅ Invitation Accepted</h1>
      <p style="color:#374151;font-size:16px;line-height:1.7;">Hi ${ownerName || 'there'},</p>
      <p style="color:#374151;font-size:16px;line-height:1.7;"><strong>${memberName}</strong> has accepted your invitation to <strong>${packageName}</strong> and is now able to access the course.</p>

      <div style="background:#F0F4F8;border-radius:12px;padding:20px 24px;margin:24px 0;">
        <p style="color:#0B1F3A;font-size:15px;font-weight:700;margin:0 0 12px;">Next Steps:</p>
        <p style="color:#374151;font-size:14px;line-height:1.7;margin:0;">You can manage your family members and track their progress from your Family Dashboard.</p>
      </div>

      ${btn('View Family Dashboard', SITE + '/family')}

      <p style="color:#374151;font-size:15px;line-height:1.7;">You can continue to invite other family members to share your seats and manage their learning progress all in one place.</p>
      ${legalFooter()}
    `)
  })
}

// ─── 13. Music Purchase Confirmation (with download link) ──────────────────
export async function sendMusicPurchaseConfirmation({ to, name, productName, productEmoji, songCount, downloadUrl }) {
  return getResend().emails.send({
    from: FROM, to,
    subject: `Your songs are ready to download — ${productName}`,
    html: wrap(`
      <h1 style="color:#0B1F3A;font-size:26px;margin:0 0 16px;">Your songs are ready! ${productEmoji || '🎵'}</h1>
      <p style="color:#374151;font-size:16px;line-height:1.7;">Hi ${name || 'there'},</p>
      <p style="color:#374151;font-size:16px;line-height:1.7;">Thank you for purchasing the <strong>${productName}</strong> song collection. Your download is ready.</p>

      <div style="background:#F0F4F8;border-radius:10px;padding:20px 24px;margin:20px 0;">
        <table style="width:100%;border-collapse:collapse;">
          <tr><td style="color:#6B7280;font-size:14px;padding:4px 0;">Collection</td><td style="color:#0B1F3A;font-size:14px;font-weight:600;text-align:right;">${productName}</td></tr>
          <tr><td style="color:#6B7280;font-size:14px;padding:4px 0;">Songs included</td><td style="color:#0B1F3A;font-size:14px;font-weight:600;text-align:right;">${songCount} MP3 files</td></tr>
          <tr><td style="color:#6B7280;font-size:14px;padding:4px 0;">Format</td><td style="color:#0B1F3A;font-size:14px;font-weight:600;text-align:right;">ZIP download</td></tr>
        </table>
      </div>

      ${btn('Download Your Songs', downloadUrl)}

      ${infoBox('<strong>Keep this email.</strong> You can use the download button above any time you need to re-download your songs. The files are yours to keep forever — play them on any device.')}

      <p style="color:#374151;font-size:15px;line-height:1.7;">If you have any trouble downloading, contact us at <a href="mailto:Support@HomeSafeEducation.com" style="color:#0EA5A0;">Support@HomeSafeEducation.com</a> and we will help straight away.</p>
      ${legalFooter()}
    `)
  })
}
...
export async function sendEmailChangedNotification({ to, name, newEmail }) {
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family: 'DM Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0B1F3A; color: #fff; border-radius: 16px; overflow: hidden;">
        <div style="padding: 40px 32px; text-align: center;">
          <h1 style="font-size: 24px; margin-bottom: 16px;">Email Address Changed</h1>
          <p style="font-size: 16px; color: #94a3b8; line-height: 1.6;">
            Hi ${name || 'there'},
          </p>
          <p style="font-size: 16px; color: #94a3b8; line-height: 1.6;">
            Your HomeSafeEducation account email has been changed to <strong style="color: #0EA5A0;">${newEmail}</strong>.
          </p>
          <p style="font-size: 16px; color: #94a3b8; line-height: 1.6;">
            If you did not make this change, please contact us immediately at support@homesafeeducation.com.
          </p>
        </div>
      </div>
    `,
