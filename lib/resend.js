...
export async function sendWelcomeEmail({ to, name }) {
  return resend.emails.send({
    ...
    html: wrap(`
      ...
      <p style="color:#374151;font-size:16px;line-height:1.7;">Your account with The Be Safe Group is now active.</p>
      ...
      <p style="color:#374151;font-size:15px;line-height:1.7;">If you have any questions before purchasing, our team is happy to help at <a href="mailto:Support@TheBeSafeGroup.com" style="color:#0EA5A0;">Support@TheBeSafeGroup.com</a>.</p>
      ${legalFooter()}
    `)
  })
}
...
export async function sendFamilyBundleWelcome({ to, name }) {
  return resend.emails.send({
    ...
    html: wrap(`
      ...
      <p style="color:#374151;font-size:15px;line-height:1.7;">If you run into any difficulty, email us at <a href="mailto:Support@TheBeSafeGroup.com" style="color:#0EA5A0;">Support@TheBeSafeGroup.com</a> and we'll help you straight away.</p>
      ${legalFooter()}
    `)
  })
}
...
export async function sendMemberInvite({ to, memberName, senderName, packageName, packageEmoji, inviteUrl }) {
  return resend.emails.send({
    from: FROM, to,
    subject: `${senderName} has given you access to ${packageName} — The Be Safe Group`,
    html: wrap(`
      ...
      <p style="color:#374151;font-size:16px;line-height:1.7;"><strong>${senderName}</strong> has purchased the <strong>${packageName}</strong> package for you through The Be Safe Group — practical, expert-led safety education you can work through at your own pace.</p>
      ...
      <p style="color:#374151;font-size:15px;line-height:1.7;">If you have any trouble, email us at <a href="mailto:Support@TheBeSafeGroup.com" style="color:#0EA5A0;">Support@TheBeSafeGroup.com</a> and we'll help straight away.</p>
      <p style="color:#9CA3AF;font-size:12px;margin-top:20px;">This invitation was sent by ${senderName} on behalf of The Be Safe Group. Your data is handled per our <a href="${SITE}/privacy" style="color:#9CA3AF;">Privacy Policy</a>. You are not obligated to accept.</p>
    `)
  })
}
...
export async function sendPasswordReset({ to, name, resetUrl }) {
  return resend.emails.send({
    from: FROM, to,
    subject: 'Reset your password — The Be Safe Group',
    html: wrap(`
      ...
      <p style="color:#374151;font-size:16px;line-height:1.7;">We received a request to reset the password for your account at TheBeSafeGroup.com.</p>
      ${btn('Reset My Password', resetUrl)}
      ${infoBox('This link expires in <strong>1 hour</strong>. If it has expired, you can request a new one at <a href="' + SITE + '/reset-password" style="color:#0EA5A0;">TheBeSafeGroup.com/reset-password</a>.')}
      ...
      <p style="color:#374151;font-size:15px;line-height:1.7;">If you believe someone is attempting to access your account, contact us immediately at <a href="mailto:Support@TheBeSafeGroup.com" style="color:#0EA5A0;">Support@TheBeSafeGroup.com</a>.</p>
...
export async function sendRefundConfirmation({ to, name, packageName, orderId, amount }) {
  return resend.emails.send({
    ...
    html: wrap(`
      ...
      <p style="color:#374151;font-size:15px;line-height:1.7;">If you have any questions, or if you'd like to explore a different course that might be a better fit, please don't hesitate to reach out at <a href="mailto:Support@TheBeSafeGroup.com" style="color:#0EA5A0;">Support@TheBeSafeGroup.com</a>.</p>
      ${legalFooter()}
    `)
  })
}
...
export async function sendPrivacyPolicyUpdate({ to, name, changeDate, changeSummary }) {
  return resend.emails.send({
    ...
    html: wrap(`
      ...
      <p style="color:#374151;font-size:15px;line-height:1.7;">The updated policy is available in full at <a href="${SITE}/privacy" style="color:#0EA5A0;">TheBeSafeGroup.com/privacy</a>.</p>
      <p style="color:#374151;font-size:15px;line-height:1.7;">If you would like to exercise any of your rights under GDPR — including access, correction, deletion, or portability of your data — please contact us at <a href="mailto:Support@TheBeSafeGroup.com" style="color:#0EA5A0;">Support@TheBeSafeGroup.com</a>. We will respond within 30 days.</p>
...
export async function sendDataRequestConfirmation({ to, name, requestType, reference }) {
  return resend.emails.send({
    ...
    html: wrap(`
      ...
      <p style="color:#374151;font-size:15px;line-height:1.7;">If you have any questions in the meantime, contact us at <a href="mailto:Support@TheBeSafeGroup.com" style="color:#0EA5A0;">Support@TheBeSafeGroup.com</a>.</p>
