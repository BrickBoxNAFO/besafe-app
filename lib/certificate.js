 ...
 * Matches the Canva template: black background, geometric triangle decorations,
 * orange/white text, HomeSafeEducation branding
 *
 * @param {Object} opts
 * @param {string} opts.recipientName - Full name of the certificate recipient
 * @param {string} opts.courseName   - Name of the completed course package (e.g. "Street Smart")
 * @param {string} opts.date         - Completion date string (e.g. "7 April 2026")
...
export async function generateCertificatePDF({ recipientName, courseName, date }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.rect(borderInset, borderInset, W - borderInset * 2, H - borderInset * 2)
        .lineWidth(1).stroke('#E8703A')
      doc.rect(borderInset + 6, borderInset + 6, W - (borderInset + 6) * 2, H - (borderInset + 6) * 2)
      ...
      doc.save()
      doc.moveTo(W / 2 - accentWidth, accentY).lineTo(W / 2 + accentWidth, accentY)
        .lineWidth(2).stroke('#E8703A')
      doc.restore()

      // ─── Logo text (HomeSafeEducation) ────────────────────────────────
      const logoY = 75
      ...
      doc.save()
      doc.opacity(0.3)
      doc.moveTo(W / 2 - 150, divY1).lineTo(W / 2 + 150, divY1)
        .lineWidth(0.5).stroke('#E8703A')
      doc.restore()
      ...
      doc.save()
      doc.opacity(0.2)
      doc.moveTo(W / 2 - 180, sigLineY).lineTo(W / 2 + 180, sigLineY)
        .lineWidth(0.5).stroke('#ffffff')
      doc.restore()
      ...
      doc.save()
      doc.opacity(0.3)
      doc.moveTo(W / 2 - 150, divY2).lineTo(W / 2 + 150, divY2)
        .lineWidth(0.5).stroke('#E8703A')
      doc.restore()
      ...
      doc.save()
      doc.opacity(0.3)
      doc.moveTo(leftCol + 20, bottomY + 28).lineTo(leftCol + 180, bottomY + 28)
        .lineWidth(0.5).stroke('#ffffff')
      doc.restore()
      ...
      doc.save()
      doc.opacity(0.3)
      doc.moveTo(rightCol + 20, bottomY + 28).lineTo(rightCol + 180, bottomY + 28)
        .lineWidth(0.5).stroke('#ffffff')
      doc.restore()
      doc.font('Helvetica').fontSize(10).fillColor('#9CA3AF')
      ...
      doc.save()
      doc.moveTo(W / 2 - accentWidth, bottomAccentY).lineTo(W / 2 + accentWidth, bottomAccentY)
        .lineWidth(2).stroke('#E8703A')
      doc.restore()
