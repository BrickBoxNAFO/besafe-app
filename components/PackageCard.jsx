...
export default function PackageCard({ pkg, owned }) {
  ...
  const quizQs = pkg.quizQs || (pkg.isGrowingBundle ? 220 : 250)
  return (
    <div className={`bg-white rounded-2xl p-6 border card-lift cursor-pointer ${owned ? 'border-teal/30 ring-1 ring-teal/20' : 'border-gray-100'}`}>
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          {pkg.isGrowingBundle && <span className="chip text-xs px-2 py-0.5 rounded-full text-white font-bold" style={{background:pkg.color}}>Bundle</span>}
